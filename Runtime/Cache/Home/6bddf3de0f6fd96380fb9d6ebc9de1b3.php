		...
		<section class="box-content">
			    ...
			    <div class="main-content clearfix">
				    <div class="now"> 
					    ...
					    <a href="<?php echo U('User/index');?>" class="movesay"><?php echo L('security');?> </a> &gt;  <?php echo L('user_contact_address_management');?> 
				    </div>
                 ...
                 <li><a href="<?php echo U('User/log');?>"><i class="fa fa-circle-o"></i> <?php echo L('user_login_log');?></a></li>
				    ...
				    <div class="tag-list-content assets_content bg_w"> 
				     <div class="safe_center clear"> 
				      	<h1> <span> <?php echo L('user_contact_address_management');?> </span> <span> <a class="right" onclick="$('#hints').slideToggle();"> <i class="fa fa-hand-o-right move fz_14 mr5"></i> <span class="fz_12 move">帮助</span> </a> </span> </h1> 
				     </div> 
	                                ...
	                                <table class="f_table" id="investLog_content">
	                                    <thead>
	                                        <tr>
													...
													<th><?php echo L('user_contact_name');?></th>
													<th><?php echo L('user_id_number');?></th>
													<th><?php echo L('user_contact_tel');?></th>
													<th><?php echo L('user_detailed_address');?></th>
													<th><?php echo L('user_addtime');?></th>
													<th><?php echo L('operate');?></th>
	                                        </tr>
	                                    </thead>
	                                    <tbody>
												<?php if(is_array($userGoodsList)): $i = 0; $__LIST__ = $userGoodsList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
...
<div id="withdrawCnyAddress" style="background:rgba(0,0,0,0.5); width:100%; height:100%; position:fixed; float:left; z-index:1000; top:0; left:0; display:none;">
    <div style="padding: 10px 40px 40px; background:#fff; border-radius:5px; max-width:520px; margin:10% auto; position: relative;">
        ...
        <h2 style="text-align:center;padding:20px 0;font-weight: bold;"><?php echo L('user_add_one_contact_address');?></h2>
...
<script>
		...
		function Update(){
			...
			if(name==""||name==null || name=="联系姓名"){
				layer.tips('<?php echo L('user_enter_note_name');?>','#goods_name',{tips:3});
			return false;
		}
		if(truename==""||truename==null || truename=="真实姓名"){
			layer.tips('<?php echo L('user_enter_contact_name');?>','#goods_truename',{tips:3});
		return false;
		}
		if(idcard==""||idcard==null || idcard=="身份证号"){
			layer.tips('<?php echo L('user_enter_id_number');?>','#goods_idcard',{tips:3});
		return false;
		}
		if(moble==""||moble==null || moble=="联系电话"){
			layer.tips('<?php echo L('user_enter_contact_tel');?>','#goods_moble',{tips:3});
		return false;
		}
	
	
		if(addr==""||addr==null || addr=="详细地址"){
			layer.tips('<?php echo L('user_enter_detailed_address');?>','#goods_addr',{tips:3});
		return false;
		}
		if(paypassword==""||paypassword==null || paypassword=="交易密码"){
			layer.tips('<?php echo L('enter_trade_password');?>','#goods_paypassword',{tips:3});
		return false;
		}
	
	
		$.post("<?php echo U('User/upgoods');?>",{name:name,truename:truename,moble:moble,addr:addr,idcard:idcard,paypassword:paypassword},function(data){
			if(data.status==1){
				layer.msg(data.info,{icon:1});
				window.location="<?php echo U('User/goods');?>";
			}else{
		...
		function UpdateB(id){
			...
			layer.ready(function () {
				...
				layer.prompt({
					...
					formType: 1,btn: ['<?php echo L('confirm');?>','<?php echo L('cancel');?>']},function(val){
						if (val) {
							//需要执行的方法
							$.post("<?php echo U('User/delgoods');?>",{id:id,paypassword:val},function(data){
								if(data.status==1){
									layer.msg(data.info,{icon:1});
									window.location="<?php echo U('User/goods');?>";
								}else{
