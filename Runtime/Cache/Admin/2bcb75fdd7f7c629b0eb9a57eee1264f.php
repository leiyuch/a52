...
<div id="main-content">
 ...
 <div id="main" class="main">
  <div class="main-title-h">
   <span class="h1-title"><?php echo ($Title); ?></span>
  </div>
  <div class="cf">
   <div class="fl">
    <a class="btn  btn-success navbar-btn btn-sm" href="<?php echo U('Money/edit');?>">新 增</a>
    <button class="btn ajax-post  btn-info navbar-btn btn-sm" url="<?php echo U('Money/status',array('method'=>'resume'));?>" target-form="ids">启 用</button>
    <button class="btn ajax-post  btn-warning navbar-btn btn-sm" url="<?php echo U('Money/status',array('method'=>'forbid'));?>" target-form="ids">禁 用</button>
    <button class="btn ajax-post confirm btn-danger navbar-btn btn-sm" url="<?php echo U('Money/status',array('method'=>'delete'));?>" target-form="ids">删 除</button>
   </div>
  </div>
   ...
   <table class="">
    ...
    <tbody>
     <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
      ...
      <td><?php echo ($C['coin'][$vo['coinname']]['title']); ?></td>
      <td><?php echo ($C['coin'][$vo['feecoin']]['title']); ?></td>
      <td><?php if(($vo["type"]) == "1"): ?><font color="green">活期</font><?php else: ?><font color="#9acd32">定期</font><?php endif; ?></td>
      <td><?php echo (NumToStr($vo['num'])); ?></td>
      <td><?php echo (NumToStr($vo['deal'])); ?></td>
      <td><?php echo ($vo['tian']); ?></td>
      <td><?php echo ($vo['lnum']); ?></td>
      <td><?php echo (NumToStr($vo['fee'])); ?></td>
      <td><?php echo ($vo['sort']*1); ?></td>
      <td><?php echo (addtime($vo['endtime'])); ?></td>
      <td><?php if(($vo["status"]) == "1"): ?>可用<?php else: ?>禁用<?php endif; ?></td>
      <td>
           <a href="<?php echo U('Money/edit?id='.$vo['id']);?>" class="btn btn-primary btn-xs">编辑</a>
           <a href="<?php echo U('Money/log',array('money_id' => $vo['id']));?>" class="btn btn-warning btn-xs">日志记录</a>
