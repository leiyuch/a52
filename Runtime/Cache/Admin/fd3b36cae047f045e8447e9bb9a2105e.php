...
<div id="main-content">
	...
	<div id="main" class="main">
		<div class="main-title-h">
			<span class="h1-title"><a href="<?php echo U('Trade/market');?>">市场列表</a> >></span>

            <span class="h1-title"><?php if(empty($data)): ?>添加市场
	            <?php else: ?>
                                                      编辑市场<?php endif; ?></span>
		</div>
		<div class="tab-wrap">
			<div class="tab-content">
				<form id="form" action="<?php echo U('Trade/marketEdit');?>" method="post" class="form-horizontal" enctype="multipart/form-data">
					<div id="tab" class="tab-pane in tab">
						<div class="form-item cf">
							<table>
								<?php if(empty($data['id'])): ?><tr class="controls">
										<td class="item-label">买方币种 :</td>
										<td><select name="buyname" class="form-control input-10x">
											<?php if(is_array($C['coin'])): $i = 0; $__LIST__ = $C['coin'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v['name']); ?>"><?php echo ($v['title']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
										</select>
										</td>
								...
								<tr class="controls">
									...
									<td>
										<input type="text" class="form-control input-10x" name="round" value="<?php echo ($data["round"]); ?>">
									</td>
								...
								<tr class="controls">
									...
									<td>
										<input type="text" class="form-control input-10x" name="fee_buy" value="<?php echo ($data["fee_buy"]); ?>">
									</td>
								...
								<tr class="controls">
									...
									<td>
										<input type="text" class="form-control input-10x" name="fee_sell" value="<?php echo ($data["fee_sell"]); ?>">
									</td>
									<td class="item-note">%有挂单的时候不能再修改</td>
								</tr>
								<tr class="controls">
									...
									<td>
										<input type="text" class="form-control input-10x" name="buy_min" value="<?php echo ($data["buy_min"]); ?>">
									</td>
								...
								<tr class="controls">
									...
									<td>
										<input type="text" class="form-control input-10x" name="buy_max" value="<?php echo ($data["buy_max"]); ?>">
									</td>
								...
								<tr class="controls">
									...
									<td>
										<input type="text" class="form-control input-10x" name="sell_min" value="<?php echo ($data["sell_min"]); ?>">
									</td>
								...
								<tr class="controls">
									...
									<td>
										<input type="text" class="form-control input-10x" name="sell_max" value="<?php echo ($data["sell_max"]); ?>">
									</td>
								...
								<tr class="controls">
									...
									<td>
										<input type="text" class="form-control input-10x" name="trade_min" value="<?php echo ($data["trade_min"]); ?>">
									</td>
								...
								<tr class="controls">
									...
									<td>
										<input type="text" class="form-control input-10x" name="trade_max" value="<?php echo ($data["trade_max"]); ?>">
									</td>
								...
								<tr class="controls">
									...
									<td><select name="invit_buy" class="form-control input-10x">
										<option value="1"
										<?php if(($data["invit_buy"]) == "1"): ?>selected<?php endif; ?>
										>开启赠送</option>
										<option value="0"
										<?php if(($data["invit_buy"]) == "0"): ?>selected<?php endif; ?>
										>禁止赠送</option>
									</select></td>
									<td class="item-note"></td>
								</tr>
								<tr class="controls">
									...
									<td><select name="invit_sell" class="form-control input-10x">
										<option value="1"
										<?php if(($data["invit_sell"]) == "1"): ?>selected<?php endif; ?>
										>开启赠送</option>
										<option value="0"
										<?php if(($data["invit_sell"]) == "0"): ?>selected<?php endif; ?>
										>禁止赠送</option>
									</select></td>
									<td class="item-note"></td>
								</tr>
