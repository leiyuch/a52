...
<div id="main-content">
    ...
    <div id="main" class="main">
        <div class="main-title-h" style="font-size: 24px;">
            ...
            <?php if(isset($_GET['name'])): ?><i class="ca"></i>[<a href="<?php echo U('Huafei/index');?>"> 记录列表</a> ]<?php endif; ?>
        </div>
        <div class="cf">
            <div class="fl"></div>
            <div class="search-form fr cf">
                <div class="sleft">
                    <form name="formSearch" id="formSearch" method="get" name="form1" action="<?php echo U('Huafei/index');?>">
                        <input type="text" name="name" class="search-input" value="<?php echo ($_GET['name']); ?>" placeholder="请输入用户名">
        ...
        <div class="data-table table-striped">
            <table class="">
                ...
                <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td>
                                <a href="<?php echo U('User/index?field=username&name='.$vo['username']);?>"><?php echo ($vo["username"]); ?></a>
                            </td>
                            <td><?php echo ($vo["moble"]); ?></td>
                            <td><?php echo ($vo["num"]); ?></td>
                            <td><?php echo ($vo["type"]); ?></td>
                            <td><?php echo ($vo["mum"]); ?>(<?php echo ($vo["type"]); ?>)</td>
                            <td><?php echo ($vo["addtime"]); ?></td>
                            <td><?php echo ($vo["endtime"]); ?></td>
                            <td>
                                <?php if(($vo['status']) == "0"): ?>正在处理<?php endif; ?>
                                <?php if(($vo['status']) == "1"): ?>已到账<?php endif; ?>
                                <?php if(($vo['status']) == "2"): ?>已撤销<?php endif; ?>
                            </td>
                            <td>
                                <?php if(($vo['status']) == "0"): ?><a href="<?php echo U('Huafei/resume?id='.$vo['id']);?>" class="btn btn-primary btn-xs ajax-get confirm">到账</a><a href="<?php echo U('Huafei/repeal?id='.$vo['id']);?>" class="btn btn-warning btn-xs ajax-get confirm">撤销</a><?php endif; ?>
                                <?php if(($vo['status']) == "1"): ?><a href="<?php echo U('Huafei/delete?id='.$vo['id']);?>" class="btn btn-danger btn-xs ajax-get confirm">删除</a><?php endif; ?>
                                <?php if(($vo['status']) == "2"): ?><a href="<?php echo U('Huafei/delete?id='.$vo['id']);?>" class="btn btn-danger btn-xs ajax-get confirm">删除</a><?php endif; ?>
