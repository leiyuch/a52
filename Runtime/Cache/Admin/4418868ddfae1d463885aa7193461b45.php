...
<div id="main-content">
	...
	<div id="main" class="main">
		<div class="main-title-h">
			<span class="h1-title"><a href="<?php echo U('Article/type');?>">文章类型</a> >></span> <span class="h1-title"><?php if(empty($data)): ?>添加文章
			<?php else: ?>
			                                                                                                                   编辑文章<?php endif; ?></span>
		</div>
		<div class="tab-wrap">
			<div class="tab-content">
				<form id="form" action="<?php echo U('Article/typeEdit');?>" method="post" class="form-horizontal" enctype="multipart/form-data">
					<div id="tab" class="tab-pane in tab">
						<div class="form-item cf">
							<table>
								...
								<tr class="controls">
									...
									<td>
										<select name="shang" class="form-control input-10x">
											<option value=""
											<?php if(($data['shang']) == ""): ?>selected<?php endif; ?>
											>顶级分类</option>
											<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"
												<?php if(($data['shang']) == $key): ?>selected<?php endif; ?>
								...
								<tr class="controls">
									<td class="item-label">类型英文标题 :</td>
									<td>
										<input type="text" class="form-control input-10x" name="en_title" value="<?php echo ($data["en_title"]); ?>">
									</td>
									<td class="item-note"></td>
								</tr>
								<tr class="controls">
								...
								<tr class="controls">
									<td class="item-label">文章英文内容 :</td>
									<td>
										<textarea name="en_content" class="form-control input-10x"><?php echo ($data["en_content"]); ?></textarea>
									</td>
									<td class="item-note">用来单页显示</td>
								</tr>
								<tr class="controls">
...
<script type="text/javascript">
	...
	KindEditor.ready(function (K) {
		editor = K.create('textarea[name="content"],textarea[name="en_content"]', {
			width: '500px',
			height: '250px',
			allowPreviewEmoticons: false,
			allowImageUpload: true,
			uploadJson: "<?php echo U('Article/typeEdit','type=images');?>",//图片上传后的处理地址
			items: [
...
//			});
	...
	<script type="text/javascript" charset="utf-8">
		//导航高亮
		highlight_subnav("<?php echo U('Article/type');?>");
	</script>