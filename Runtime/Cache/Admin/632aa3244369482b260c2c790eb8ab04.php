		...
		<div class="main-title-h">
			...
			<select id="name-select" name="market" class="navbar-btn  form-control select select-default select-sm" style="       height: 30px; width: 150px;min-width:150px;">
				<?php if(is_array($C['market'])): $i = 0; $__LIST__ = $C['market'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['name']); ?>"
					<?php if(($vo['name']) == $market): ?>selected<?php endif; ?>
					><?php echo ($vo['title']); ?>					</option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select>

			<button class="btn btn-success " onclick="chk_xin('<?php echo ($market); ?>');" style="    margin-top: -10px; margin-left: 5px;">重新初始化当前市场统计</button>
		...
		<div class="row state-overview">
			<div class="col-lg-3 col-sm-6">
				<section class="panel">
					...
					<div class="value">
						<h1 class="count" style="font-size: 20px;"><?php echo ($data['trance_ci']); ?></h1>
						<p>累计成交次数</p>
					</div>
				</section>
			</div>
			<div class="col-lg-3 col-sm-6">
				<section class="panel">
					...
					<div class="value">
						<h1 class=" count2" style="font-size: 20px;"><?php echo ($data['trance_num']*1); ?></h1>
						<p>累计交易量</p>
					</div>
				</section>
			</div>
			<div class="col-lg-3 col-sm-6">
				<section class="panel">
					...
					<div class="value">
						<h1 class=" count3" style="font-size: 20px;"><?php echo ($data['trance_fee']*1); ?></h1>
						<p>累计手续费</p>
						<h1 class=" count3" style="font-size: 12px;">买入<?php echo ($data['trance_buyfee']*1); ?></h1>
						<h1 class=" count3" style="font-size: 12px;">卖出<?php echo ($data['trance_sellfee']*1); ?></h1>
					</div>
				</section>
			</div>
			<div class="col-lg-3 col-sm-6">
				<section class="panel">
					...
					<div class="value">
						<h1 class=" count4" style="font-size: 20px;"><?php echo ($data['trance_mum']*1); ?></h1>
						<p>累计交易额</p>
					</div>
				</section>
			</div>
...
<script>
	...
	function chk_xin($market) {
		$.get("<?php echo U('Index/marketSet');?>", {
			market: $market,
		}, function (data) {
			if (data.status == 1) {
...
//			});
	...
	<script type="text/javascript" charset="utf-8">
		//导航高亮
		highlight_subnav("<?php echo U('Index/market');?>");
	</script>