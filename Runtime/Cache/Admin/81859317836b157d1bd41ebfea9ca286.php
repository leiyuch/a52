...
<div id="main-content">
	...
	<div id="main" class="main">
		...
		<div class="cf">
			<div class="fl">
                <a class="btn btn-success" href="<?php echo U('Trade/index_export', array('start_time'=>I('get.start_time'), 'end_time'=>I('get.end_time'), 'market'=>I('get.market'), 'status'=>I('get.status'), 'field'=>I('get.field'), 'name'=>I('get.name')));?>">导出</a>
            </div>
			<div class="search-form fr cf">
				<div class="sleft">
					<form name="formSearch" id="formSearch" method="get" name="form1">
						<input type="text" class="search-input form-control" name="start_time" value="<?php echo ($_GET['start_time']); ?>" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})" placeholder="开始时间" style="margin-right: 10px;width: 160px;">
                        <input type="text" class="search-input form-control" name="end_time" value="<?php echo ($_GET['end_time']); ?>" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})" placeholder="结束时间" style="margin-right: 10px;width: 160px;">
						<select style="width: 160px; float: left; margin-right: 10px;" name="market" class="form-control">
							<option value=""
							<?php if(empty($_GET['market'])): ?>selected<?php endif; ?>
							>交易市场</option>
							<?php if(is_array($C['market'])): $i = 0; $__LIST__ = $C['market'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['name']); ?>"
								<?php if(($vo['name']) == $_GET['market']): ?>selected<?php endif; ?>
		...
		<div class="data-table table-striped">
			<table class="">
				...
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php echo ($vo['market']); ?></td>
							<td><?php echo (Num($vo['price'])); ?></td>
							<td><?php echo (Num($vo['num'])); ?></td>
							<td><?php echo (Num($vo['deal'])); ?></td>
							<td><?php echo (Num($vo['mum'])); ?></td>
							<td>
								<?php if(($vo["type"]) == "1"): ?>买入
									<?php else: ?>
									卖出<?php endif; ?>
							...
							<td>
								<?php if(($vo["status"]) == "1"): ?>已成交<?php endif; ?>
								<?php if(($vo["status"]) == "0"): ?>交易中<?php endif; ?>
								<?php if(($vo["status"]) == "2"): ?>撤销<?php endif; ?>
							</td>
							<td>
								
								<?php if(($vo["market"]) == "sgc_cny"): if(($vo["status"]) == "0"): if(($vo["type"]) == "1"): if(($vo["status1"]) == "1"): ?><a href="<?php echo U('Trade/ofcbuy_chexiao?id='.$vo['id']);?>" class="ajax-get btn btn-primary btn-xs">撤销 </a><?php endif; ?> 
								
								<?php if(($vo["status1"]) == "2"): ?><a href="<?php echo U('Trade/Noofcfukuan?id='.$vo['id']);?>" class="ajax-get btn btn-primary btn-xs">未付款 </a></a><?php endif; endif; ?>
								<?php if(($vo["type"]) == "2"): if(($vo["status1"]) == "2"): ?><a href="<?php echo U('Trade/upTrade2?id='.$vo['id']);?>" class="ajax-get btn btn-primary btn-xs">确认收款 </a></a><?php endif; ?> 
	                    	<?php if(($vo["status1"]) == ""): ?><a href="<?php echo U('Trade/chexiao?id='.$vo['id']);?>" class="ajax-get btn btn-primary btn-xs">撤销 </a><?php endif; endif; endif; ?> 
								<?php if(($vo["imgurl"]) == ""): else: ?> 	
		                            <a  id="<?php echo ($vo["id"]); ?>" class="ajax-get btn btn-primary btn-xs" onclick="upimg('<?php echo ($vo["imgurl"]); ?>')" href="javascript:void(0);"> 查看凭证</a><?php endif; ?> 
		
		
		
								  <?php else: ?> 
								  <?php if(($vo["status"]) == "0"): ?><a href="<?php echo U('Trade/chexiao?id='.$vo['id']);?>" class="ajax-get btn btn-primary btn-xs">撤销 </a><?php endif; endif; ?> 
							</td>
