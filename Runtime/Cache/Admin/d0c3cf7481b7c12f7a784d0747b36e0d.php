...
<div id="main-content">
	...
	<div id="main" class="main">
		...
		<nav role="navigation" class="cf">
			<div class="fl">
				<a id="add-group" class="btn btn-success " href="<?php echo U('User/authEdit');?>">新 增</a>
				<a url="<?php echo U('User/authStatus',array('type'=>'resume'));?>" class="btn ajax-post btn-info " target-form="ids">启 用</a>
				<a url="<?php echo U('User/authStatus',array('type'=>'forbid'));?>" class="btn ajax-post btn-warning " target-form="ids">禁 用</a>
				<a url="<?php echo U('User/authStatus',array('type'=>'del'));?>" class="btn ajax-post confirm btn-danger " target-form="ids">删 除</a>
				<a url="<?php echo U('User/authStart');?>" class="btn ajax-post confirm btn-primary" target-form="ids">重新初始化权限</a>
		...
		<div class="data-table table-striped">
			<table class="">
				...
				<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td>
								<a class="btn btn-warning btn-xs" href="<?php echo U('User/authAccess?group_name='.$vo['title'].'&group_id='.$vo['id']);?>">访问授权</a>
								<a class="btn btn-info btn-xs" href="<?php echo U('User/authUser?group_name='.$vo['title'].'&group_id='.$vo['id']);?>">成员授权</a>
							</td>
							<td>
							<?php if(($vo["status"]) == "0"): echo ($vo["status_text"]); endif; ?>
							<?php if(($vo["status"]) == "1"): echo ($vo["status_text"]); endif; ?>
							</td>
							<td>
								<a href="<?php echo U('User/authEdit?id='.$vo['id']);?>" class="btn btn-primary btn-xs">编辑</a>
							</td>
