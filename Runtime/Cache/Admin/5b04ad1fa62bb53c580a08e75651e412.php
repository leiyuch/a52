...
<div id="main-content">
	...
	<div id="main" class="main">
		<div class="main-title-h">
			<span class="h1-title"><a href="<?php echo U('Issue/index');?>">认购管理</a> >></span> <span class="h1-title"><?php if(empty($data)): ?>添加认购
			<?php else: ?>
			                                                                                                                  编辑认购<?php endif; ?></span>
		</div>
		<div class="tab-wrap">
						...
						<div class="form-item cf">
							<table>
								...
								<tr class="controls">
									...
									<td><select name="coinname" class="form-control input-10x">
										<?php if(is_array($C['coin_list'])): $i = 0; $__LIST__ = $C['coin_list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v['name']); ?>"
											<?php if(($data['coinname']) == $v['name']): ?>selected<?php endif; ?>
											><?php echo ($v['title']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
									</select></td>
									<td class="item-note">用户购买后得到的</td>
								</tr>
								<tr class="controls">
									<td class="item-label">付款币种 :</td>
									<td><select name="buycoin" class="form-control input-10x">
										<?php if(is_array($C['coin'])): $i = 0; $__LIST__ = $C['coin'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v['name']); ?>"
											<?php if(($data['buycoin']) == $v['name']): ?>selected<?php endif; ?>
											><?php echo ($v['title']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
									</select></td>
									<td class="item-note">用户购买使用的</td>
								</tr>
								<tr class="controls">
									<td class="item-label">认购总量 :</td>
									<td>
								...
								<tr class="controls">
									...
									<td>
										<input type="text" class="form-control  input-10x " name="deal" value="<?php echo ($data['deal']*1); ?>">
									</td>
									<td class="item-note"></td>
								</tr>
								<tr class="controls">
								...
								<tr class="controls">
									...
									<td>
										<input type="text" class="form-control  input-10x " name="limit" value="<?php echo ($data["limit"]); ?>">
									</td>
									<td class="item-note"></td>
								</tr>
								<tr class="controls">
									...
									<td>
										<input type="text" class="form-control  input-10x " name="time" value="<?php echo ($data['time']); ?>" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})">
									</td>
									<td class="item-note"></td>
								</tr>
								<tr class="controls">
								...
								<tr class="controls">
									...
									<td>
										<input type="text" class="form-control  input-10x " name="ci" value="<?php echo ($data['ci']); ?>">
									</td>
								...
								<tr class="controls">
									...
									<td>
										<input type="text" class="form-control  input-10x " name="jian" value="<?php echo ($data['jian']); ?>">
									</td>
									<td class="item-note">小时</td>
								</tr>
								<tr class="controls">
									<td class="item-label">单次最小 :</td>
									<td>
								...
								<tr class="controls">
									...
									<td><select name="invit_coin" class="form-control  input-10x">
										<?php if(is_array($C['coin'])): $i = 0; $__LIST__ = $C['coin'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v['name']); ?>"
											<?php if(($data['invit_coin']) == $v['name']): ?>selected<?php endif; ?>
											><?php echo ($v['title']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
									</select></td>
