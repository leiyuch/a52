...
<div id="main-content">
 ...
 <div id="main" class="main">
  <div class="main-title-h">
   <span class="h1-title">APP广告板块管理</span>
  </div>
  <div class="cf">
   <div class="fl">
    <a class="btn btn-success navbar-btn btn-sm" href="<?php echo U('App/adsblock_edit');?>">新 增</a>
    <button class="btn ajax-post btn-info navbar-btn btn-sm" url="<?php echo U('App/adsblock_edit_status',array('method'=>'resume'));?>" target-form="ids">启 用</button>
    <button class="btn ajax-post btn-warning navbar-btn btn-sm" url="<?php echo U('App/adsblock_edit_status',array('method'=>'forbid'));?>" target-form="ids">禁 用</button>
    <button class="btn ajax-post confirm btn-danger navbar-btn btn-sm" url="<?php echo U('App/adsblock_edit_status',array('method'=>'delete'));?>" target-form="ids">删 除</button>
   </div>
  </div>
   ...
   <table class="">
    ...
    <tbody>
     <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
       ...
       <td><span style="color: #1abc9c;"><?php echo ($rankMap[$vo['rank']]); ?></span> 以上</td>
       <td><span style="color: #1abc9c;"><?php echo ($vo["remain"]); ?></span> s</td>
       <td><img src="<?php echo ($vo["img"]); ?>" width="60"  style="margin-bottom: 20px;"></td>
       <td><?php echo (addtime($vo["addtime"])); ?></td>
       <td><?php if(($vo["status"]) == "1"): ?>可用<?php else: ?>禁用<?php endif; ?></td>
       <td>
        <a href="<?php echo U('App/ads_list?block_id='.$vo['id']);?>" class="btn btn-primary btn-xs">旗下广告</a>
        <a href="<?php echo U('App/adsblock_edit?id='.$vo['id']);?>" class="btn btn-primary btn-xs">编辑</a>
