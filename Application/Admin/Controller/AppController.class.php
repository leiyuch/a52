...
class AppController extends AdminController
{
	...
	public function ads_user()
	{
		...
		$where = array();
		$count = $this->Model->join('movesay_user ON movesay_user.id = movesay_app_vipuser.uid')->join('movesay_app_vip ON movesay_app_vip.id = movesay_app_vipuser.vip_id')->field('movesay_user.username,movesay_app_vipuser.*,movesay_app_vip.name as vip_name,movesay_app_vip.tag')->where($where)->count();
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
		$list = $this->Model->join('movesay_user ON movesay_user.id = movesay_app_vipuser.uid')->join('movesay_app_vip ON movesay_app_vip.id = movesay_app_vipuser.vip_id')->field('movesay_user.username,movesay_app_vipuser.*,movesay_app_vip.name as vip_name,movesay_app_vip.tag')->where($where)->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
		$this->assign('list', $list);
