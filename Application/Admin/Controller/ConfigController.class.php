	...
	public function checkUpdata()
	{
		if (!S(MODULE_NAME . CONTROLLER_NAME . 'checkUpdata')) {
			...
			if (!in_array('footer_logo', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` ADD COLUMN `footer_logo` VARCHAR(200)  NOT NULL   COMMENT \' \' AFTER `id`;');
			}

			if (in_array('mycz_invit_3', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `mycz_invit_3`;');
			}

			if (in_array('mycz_invit_2', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `mycz_invit_2`;');
			}

			if (in_array('mycz_invit_1', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `mycz_invit_1`;');
			}

			if (in_array('mycz_invit_coin', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `mycz_invit_coin`;');
			}

			if (in_array('mycz_fee', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `mycz_fee`;');
			}

			if (in_array('mycz_min', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `mycz_min`;');
			}

			if (in_array('mycz_max', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `mycz_max`;');
			}

			if (in_array('mycz_text_index', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `mycz_text_index`;');
			}

			if (in_array('mycz_text_log', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `mycz_text_log`;');
			}

			if (in_array('mytx_text_index', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `mytx_text_index`;');
			}

			if (in_array('mytx_text_log', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `mytx_text_log`;');
			}

			if (in_array('trade_text_index', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `trade_text_index`;');
			}

			if (in_array('trade_text_entrust', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `trade_text_entrust`;');
			}

			if (in_array('issue_text_index', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `issue_text_index`;');
			}

			if (in_array('issue_text_log', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `issue_text_log`;');
			}

			if (in_array('issue_text_plan', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `issue_text_plan`;');
			}

			if (in_array('invit_text_index', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `invit_text_index`;');
			}

			if (in_array('invit_text_award', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` DROP COLUMN `invit_text_award`;');
			}
