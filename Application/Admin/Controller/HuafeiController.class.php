...
class HuafeiController extends AdminController
{
	...
	public function config()
	{
		...
		if (!in_array('huafei_appkey', $Config_DbFields)) {
			M()->execute('ALTER TABLE `movesay_config` ADD COLUMN `huafei_appkey` VARCHAR(200)  NOT NULL   COMMENT \'名称\' AFTER `id`;');
		}

		if (!in_array('huafei_openid', $Config_DbFields)) {
			M()->execute('ALTER TABLE `movesay_config` ADD COLUMN `huafei_openid` VARCHAR(200)  NOT NULL   COMMENT \'名称\' AFTER `id`;');
		}

		if (!in_array('huafei_zidong', $Config_DbFields)) {
			M()->execute('ALTER TABLE `movesay_config` ADD COLUMN `huafei_zidong` VARCHAR(200)  NOT NULL   COMMENT \'名称\' AFTER `id`;');
		}
