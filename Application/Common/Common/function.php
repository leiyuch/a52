 ...
 */  
 function sendMsg($mobile,$code){  
    ...
    $accessKeyId ='LTAIFJ1KZy80zAam';// $config['alims_appkey'];  
    $accessKeySecret ='9d8JjOxxun1A5mYi0rr3MvQweGxfG6';//  $config['alims_appsecret'];  
    $templateParam = array("code"=>$code);           //模板变量替换  
    $signName ='欧亚国际';// (empty($config['signname'])?'阿里大于测试专用':$config['signname']);  
    $templateCode ='SMS_109360308';// $config['notice_templateid'];   //短信模板ID  
...
function send_moble($moble, $content)
{
	...
	$url =  'http://utf8.api.smschinese.cn?Uid=sgc123456&Key=b2968e15fcf0e211f1ad&smsMob=' . $moble . '&smsText=' . $content;
...
function check($data, $rule = NULL, $ext = NULL)
{
	...
	$validate['xnb'] = '/^[a-zA-Z]$/';
    $taiwanreg='/^[A-Z][0-9]{9}$/';
        $xianggangreg='/^[A-Z][0-9]{6}\（[0-9A]\）$/';
		 $xianggangreg1='/^[A-Z][0-9]{6}\([0-9A]\)$/';
        $aomenreg='/^[157][0-9]{6}\([0-9]\)$/';
		  $aomenreg1='/^[157][0-9]{6}\（[0-9]\）$/';
		  	$validate['number1'] = '/^[0-9]*$/';
	if (isset($validate[strtolower($rule)])) {
		...
		if($rule=='idcard')
		{
			...
			return preg_match($rule, $data)||preg_match($taiwanreg, $data)||preg_match($xianggangreg, $data)||preg_match($aomenreg, $data)||preg_match($aomenreg1, $data)||preg_match($xianggangreg1, $data);
		}
