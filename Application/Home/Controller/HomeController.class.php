...
class HomeController extends \Think\Controller
{
	protected function _initialize()
	{
		...
		if (!session('userId')) {
			...
			// 		if ($user['id']) {
			// 			$ecshecom_redirect = 'http://m.eurasiant.com'."/Login/register/invit/".$user['id'];
			// 		}else{
			// 			$ecshecom_redirect = 'http://m.eurasiant.com';
			// 		}
			// 	}else{
			// 		$ecshecom_redirect = 'http://m.eurasiant.com';
			// 	}
			// 	header("Location:".$ecshecom_redirect);
			// 	die();
			// } 
	...
	//检查用户是否被锁定
	public function Check_IsSuoDing($userid)
	{
		   $r=0;
