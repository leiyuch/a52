...
class ChartController extends HomeController
{
	...
	{
		$mo = M();
		$buy = $mo->query('select id,price,sum(num-deal)as nums from movesay_trade  where status=0 and type=1 and market =\'' . $market . '\' group by price order by price desc limit 100;');
		$data = '';
		...
		if ($buy) {
			...
			foreach ($buy as $k => $v) {
				$data .= '<tr><td class="buy"  width="50">' .L('trade_buy_1'). ($k + 1) . '</td><td class="buy"  width="80">' . floatval($v['price']) . '</td><td class="buy"  width="120">' . floatval($v['nums']) . '</td><td  width="100"><span class="buySpan" style="width: ' . ((($maxNums < $v['nums'] ? $maxNums : $v['nums']) / $maxNums) * 100) . 'px;" ></span></td></tr>';
			}
		}
	...
	{
		$mo = M();
		$sell = $mo->query('select id,price,sum(num-deal)as nums from movesay_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit 100;');
		$data = '';
		...
		if ($sell) {
			...
			foreach ($sell as $k => $v) {
				$data .= '<tr><td class="sell"  width="50">' .L('trade_sell_1').  ($k + 1) . '</td><td class="sell"  width="80">' . floatval($v['price']) . '</td><td class="sell"  width="120">' . floatval($v['nums']) . '</td><td style="width: 100px;"><span class="sellSpan" style="width: ' . ((($maxNums < $v['nums'] ? $maxNums : $v['nums']) / $maxNums) * 100) . 'px;" ></span></td></tr>';
			}
		}
