...
private $options = array(
		...
		'host' => 'smtp.meunite.com',
		'port' => '465',
		'username' => 'meunite@meunite.com',
		'password' => 'Meilianwang123',
		'sender_email' => 'meunite@meunite.com',
		'sender_name' => 'meunite'
	);

		
	// $mail->Host = "smtp.yeah.net"; 						
	// $mail->Username = "sumikeji@yeah.net";   				
	// $mail->Password = "2016Sumikeji";       
	public function emailcode()
	{
		...
		if (IS_POST) {
			...
			$code = rand(111111, 999999);
			//session('code'.$input['email'], $code);
			$content = '您正在进行注册操作，您的验证码是' . $code;

			$res = setPostEmail($this->options['host'], $this->options['username'], $this->options['password'], $this->options['sender_name'],
			$input['email'], '验证码', $content);
		// $res = setPostEmail("smtp.yeah.net", "sumikeji@yeah.net", "2016Sumikeji", $this->options['sender_name'],$input['email'], '验证码', $content);
			if (empty($res)) {
				$this->success('邮箱验证码已发送到你的邮箱，请查收');
			}
			else {
				$this->error('邮箱验证码发送失败，请重新点击发送'.$input['email'].$this->options['host'].$res);
			}
		}
	}
