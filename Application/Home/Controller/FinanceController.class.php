...
class FinanceController extends HomeController
{
    public function index()
    {
        ...
        //显示用户积分
        $this->assign('integral', round($UserCoin['integral'], 2) * 1);
    ...
    public function mytxUp($moble_verify, $num, $paypassword, $type)
    {
        ...
        if (md5($paypassword) != $user['paypassword']) {
            $this->error(L('finance_trade_password_error'));
        }
    ...
    public function myzr($coin = null)
    {
        ...
        if (!$Coin['zr_jz']) {
            $qianbao = L('finance_ban_into_currency');
        } else {
            $qbdz = $coin . 'b';
            ...
            if (!$user_coin[$qbdz]) {
                if ($Coin['type'] == 'rgb') {
                    ...
                    if (!$rs) {
                        $this->error(L('finance_create_wallet_address_error'));
                    }
                }
                ...
                if ($Coin['type'] == 'qbb') {
                    ...
                    if (!isset($json['version']) || !$json['version']) {
                        $this->error(L('finance_wallet_link_fail'));
                    }
                    ...
                    if (!$rs) {
                        $this->error(L('finance_add_wallet_address_error'));
                    }
                }
            } else {
    ...
    public function myzc($coin = null)
    {
        ...
        if (!$coin_list[$coin]['zc_jz']) {
            $this->assign('zc_jz', L('finance_ban_out_currency'));
        } else {
    ...
    public function upmyzc1($coin, $num, $addr, $paypassword, $moble_verify)
    {

        if (!userid()) {
            $this->error(L('finance_please_login_first'));
        }
        //判断是否被锁定
        ...
        if (!check($num, 'currency')) {
            $this->error(L('finance_amount_format_error'));
        }

        if (!check($addr, 'dw')) {
            $this->error(L('finance_wallet_address_format_error'));
        }
        ...
        if (!check($coin, 'n')) {
            $this->error(L('finance_currency_format_error'));
        }

        if (!C('coin')[$coin]) {
            $this->error(L('finance_currency_error'));
        }
        ...
        if ($num < $myzc_min) {
            $this->error(L('finance_transfer_minimum_limit'));
        }

        if ($myzc_max < $num) {
            $this->error(L('finance_transfer_maximum_limit'));
        }
        ...
        if ($user_coin[$coin] < $num) {
            $this->error(L('finance_available_balance'));
        }
        ...
        if ($fee_user) {
            ...
            if ($mum < 0) {
                $this->error(L('finance_transfer_fee_error'));
            }

            if ($fee < 0) {
                $this->error(L('finance_transfer_fee_set_error'));
            }
        } else {
        ...
        if ($Coin['type'] == 'rgb') {
            ...
            if ($mo->table('movesay_user_coin')->where(array($qbdz => $addr))->find()) {
                ...
                if (!$peer) {
                    $this->error(L('finance_transfer_address_not_exist'));
                }
                ...
                if (check_arr($rs)) {
                    ...
                    $this->success(L('finance_transfer_succeed'));
                } else {
                    $mo->execute('rollback');
                    $this->error(L('finance_transfer_failed'));
                }
            } else {
                ...
                if (check_arr($rs)) {
                    ...
                    if ($auto_status) {
                        ...
                        $sign = 'Amount=' . $num . '&WalletAddress=' . $addr . '&outWalletAddress=' . $user_coin[$coin . 'b'] . '&K4TZOkwK17x0hAS4';

                        $post_data['WalletAddress']    = $addr;
                        ...
                        curl_close($ch);  
                        session('myzc_verify', null);
                        $this->success(L('finance_transfer_out_succeed'));
                        // if ($flag == 1) {
                        //     $mo->execute('commit');
                        //     $mo->execute('unlock tables');
                        //     session('myzc_verify', null);
                        ...
                        //     $ss = $arr['str'];
                        //     $this->error(L('finance_transfer_out_failed') . $ss . $flag);
                        // }
                    }

                } else {
                    $mo->execute('rollback');
                    $this->error(L('finance_transfer_out_failed'));
                }
            }
        }
...
//判断是否被锁定
        ...
        if ($Coin['type'] == 'rgb') {
            ...
            if (!$peer) {
                $this->error(L('finance_currency_address_does_not_exist'));
            }
        ...
        if ($Coin['type'] == 'qbb') {
            ...
            if ($mo->table('movesay_user_coin')->where(array($qbdz => $addr))->find()) {
                ...
                if (!$valid_res['isvalid']) {
                    $this->error($addr . L('finance_invalid_wallet_address'));
                }
                ...
                if (check_arr($rs)) {
                    if ($auto_status) {
                        ...
                        if (!$flag) {
                            $this->error(L('finance_server_failed'));
                        } else {
                            $this->success(L('finance_transfer_out_succeed'));
                        }
                    }
                    ...
                    if ($auto_status) {
                        ...
                        $mo->execute('unlock tables');
                        session('myzc_verify', null);
                        $this->success(L('finance_transfer_wait_audit'));
                    }
                } else {
                    $mo->execute('rollback');
                    $this->error(L('finance_transfer_out_failed'));
