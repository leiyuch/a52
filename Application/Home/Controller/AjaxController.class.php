...
class AjaxController extends HomeController
{
    ...
    public function getJsonPrice($market = null, $ajax = 'json')
    {
        $data = (APP_DEBUG ? null : S('getJsonTop' . $market));
    ...
    public function getDepth($market = null, $trade_moshi = 1, $ajax = 'json')
    {
        ...
        if (!$data_getDepth[$market][$trade_moshi]) {
            ...
            if ($trade_moshi == 1) {
                ...
                // $sell = array_reverse($mo->query('select id,price,sum(num-deal)as nums from movesay_trade  where status=0 and status1 is null and type=2 and market=\'' . $market . '\' and userid<>' . userid() . ' order by price asc limit 50;'));

                //只显示一个国家的
                $country = M('User')->where(array('id' => userid()))->getField('country');
                if ($country) {
                    $sell = array_reverse($mo->query('select a.userid, a.id,a.price,(a.num-a.deal)as nums from movesay_trade a inner join movesay_user b on a.userid=b.id  where a.status=0 and a.status1 is null and a.type=2 and a.market =\'' . $market . '\'  and b.country =\'' . $country . '\' and a.userid<>' . userid() . ' order by a.price asc limit ' . $limt . ';'));
                } else {
                    $sell = array_reverse($mo->query('select userid, id,price,(num-deal)as nums from movesay_trade where status=0 and status1 is null and type=2 and market =\'' . $market . '\' and userid<>' . userid() . ' order by price asc limit ' . $limt . ';'));
                }
            ...
            if ($trade_moshi == 4) {
                $buy  = null;
                $sell = array_reverse($mo->query('select userid, id,price,sum(num-deal)as nums from movesay_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit ' . $limt . ';'));
            }
            ...
            if ($sell) {
                foreach ($sell as $k => $v) {
                    $data['depth']['sell'][$k] = array(floatval($v['price'] * 1), floatval($v['nums'] * 1), floatval($v['id'] * 1), floatval($v['userid'] * 1));
                }
            } else {
                $data['depth']['sell'] = '';
            }
    ...
    public function getEntrustAndUsercoin($market = null, $ajax = 'json')
    {
        ...
        $result = M()->query('select id,price,mum,num,deal,mum,type,fee,status,addtime from movesay_trade where status=0 and market=\'' . $market . '\' and userid=' . userid() . ' order by id desc limit 10;');
        ...
        if ($result) {
            foreach ($result as $k => $v) {
                ...
                $data['entrust'][$k]['mum']     = round($v['mum']);
            }
        } else {
            $data['entrust'] = null;
        }
        ...
        if ($country) {
            ...
            $result1 = M()->query('select a.* from movesay_trade a inner join movesay_user b on a.userid=b.id  where a.status=0 and a.status1 is null and a.type=2 and a.market=\'' . $market . '\' and b.country=\'' . $country . '\' and a.userid<>' . userid() . ' order by a.price asc limit 50;');
        } else {
            $result1 = M()->query('select userid, id,price,num,deal,mum,type,fee,status,addtime from movesay_trade where status=0 and status1 is null and type=2 and market=\'' . $market . '\' and userid<>' . userid() . ' order by price asc limit 50;');
        }

        if ($result1) {
            foreach ($result1 as $k => $v) {
                $data['entrust1'][$k]['addtime'] = date('m-d H:i:s', $v['addtime']);
                $data['entrust1'][$k]['type']    = $v['type'];
                $data['entrust1'][$k]['price']   = $v['price'] * 1;
                $data['entrust1'][$k]['num']     = round($v['num'], 6);
                $data['entrust1'][$k]['deal']    = round($v['deal'], 6);
                $data['entrust1'][$k]['id']      = round($v['id']);
                $data['entrust1'][$k]['userid']  = $v['userid'];
            }
        } else {
            $data['entrust1'] = null;
        }
    ...
    //这个是后来加的,获取所有 的 卖单不包含自己的

    public function getEntrustAndOutUsercoin($market = null, $ajax = 'json')
    {
        if (!userid()) {
            return null;
        }
    ...
    public function getUserBank($userid = null, $ajax = 'json')
    {

        $data     = (APP_DEBUG ? null : S('UserBankOne'));
        $UserBank = M('UserBank')->where(array('userid' => $userid, 'status' => 1))->order('id desc')->limit(1)->select();
        $User     = M('User')->where(array('id' => $userid))->order('id desc')->find();
        if ($UserBank) {
            foreach ($UserBank as $k => $v) {
                $data['BankOne'][$k]['bank']     = $v['bank'];
                $data['BankOne'][$k]['bankprov'] = $v['bankprov'];
                $data['BankOne'][$k]['bankcity'] = $v['bankcity'];
                $data['BankOne'][$k]['bankaddr'] = $v['bankaddr'];
                $data['BankOne'][$k]['truename'] = $v['truename'];
                $data['BankOne'][$k]['bankcard'] = $v['bankcard'];
                $data['BankOne'][$k]['truename'] = $User['truename'];
                $data['BankOne'][$k]['moble']    = $User['moble'];
            }
