...
class QueueController extends HomeController
{
    ...
    //三小时没付款自动取消买家订单
    public function autoCandelBuy()
    {

        $mo = M();
        $mo->execute('set autocommit=0');
        $mo->execute('lock tables movesay_trade write,movesay_finance write,movesay_user_coin write');

        $t     = time() - 3600 * 48;
        $List1 = $mo->table('movesay_trade')->where("market='sgc_cny' and status=0 and type=1 and  status1<>2 and  addtime< " . $t)->count();
        $List  = M('Trade')->where("market='sgc_cny' and status=0 and type=1 and  status1<>2 and  addtime< " . $t)->select();

        if ($List1 > 0) {
            foreach ($List as $k => $trade) {
                ...
                //修改匹配的记录
                $rs = $mo->table('movesay_trade')->where(array('id' => $trade['buyid']))->setField('status1', null);
                $rs = $mo->table('movesay_trade')->where(array('id' => $trade['buyid']))->setField('buyuserid', null);
                $rs = $mo->table('movesay_trade')->where(array('id' => $trade['buyid']))->setField('buyid', null);
                ...
                $finance_hash          = md5($trade['userid'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $save_buy_rmb . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . 'auth.meunite.com');
    ...
    //三小时没有确认收款系统锁定双方帐户
    public function autolockuser()
    {

        $mo = M();
        $mo->execute('set autocommit=0');
        $mo->execute('lock tables movesay_trade write,movesay_user write');

        $t     = time() - 3600 * 5;
        $List1 = $mo->table('movesay_trade')->where("market='sgc_cny' and status=0 and    status1=2 and  paytime< " . $t)->count();
        $List  = $mo->table('movesay_trade')->where("market='sgc_cny' and status=0 and    status1=2 and  paytime< " . $t)->select();

        if ($List1 > 0) {
            foreach ($List as $k => $trade) {
                //修改买的交易状态为已撤销
                $rs[] = $mo->table('movesay_user')->where(array('id' => $trade['userid']))->setField('status', 2);
    ...
    public function chart()
    {
        ...
        // }
        // $this->setTradeJson('btc_cny');
        // $this->setTradeJson('ltc_cny');
        //$this->setTradeJson('eth_cny');
        $this->setTradeJson('sgc_cny');
