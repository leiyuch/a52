...
class UserController extends CommonController
{
	public function userinfo()
	{
		...
		foreach ($CoinList as $k => $v) {
			if ($v['name'] == 'cny') {
				...
				if (isset($Market[$v['name'] . '_cny'])) {
					$jia = $Market[$v['name'] . '_cny']['new_price'];
					$marketid = $Market[$v['name'] . '_cny']['id'];
				}
				else {
					$jia = 1;
					$marketid = 0;
	...
	public function upmytx($moble_verify, $num, $paypassword, $type)
	{
		...
		$rs = array();
		$rs[] = $mo->table('movesay_user_coin')->where(array('userid' => $uid))->setDec('cny', $num);
		$rs[] = $mo->table('movesay_mytx')->add(array('userid' => $uid, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'name' => $userBank['name'], 'truename' => $user['truename'], 'bank' => $userBank['bank'], 'bankprov' => $userBank['bankprov'], 'bankcity' => $userBank['bankcity'], 'bankaddr' => $userBank['bankaddr'], 'bankcard' => $userBank['bankcard'], 'addtime' => time(), 'status' => 0));
	...
	public function upmyzc($coin_id, $moble_verify, $num, $paypassword, $addr)
	{
		...
		if ($Coin['type'] == 'rgb') {
			...
			$rs = array();
			$rs[] = $mo->table('movesay_user_coin')->where(array('userid' => $uid))->setDec($coin, $num);
			...
			$rs[] = $mo->table('movesay_myzc')->add(array('userid' => $uid, 'username' => $addr, 'coinname' => $coin, 'txid' => md5($addr . $user_coin[$coin . 'b'] . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 1));
		...
		if ($Coin['type'] == 'qbb') {
			...
			if ($mo->table('movesay_user_coin')->where(array($qbdz => $addr))->find()) {
				...
				$mo->execute('lock tables  movesay_user_coin write  , movesay_myzc write ,movesay_myzr write, movesay_myzc_fee write');
				$rs = array();
				$rs[] = $r = $mo->table('movesay_user_coin')->where(array('userid' => $uid))->setDec($coin, $num);
				debug(array('res' => $r, 'lastsql' => $mo->table('movesay_user_coin')->getLastSql()), '更新用户人民币');
				$rs[] = $aid = $mo->table('movesay_myzc')->add(array('userid' => $uid, 'username' => $addr, 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => $auto_status));
